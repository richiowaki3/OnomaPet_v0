...
## 2. 4ノード 3D CH-4 エフォート物理メッシュモデル (4-Node Pure CH-4 Elastic Mesh)

過剰な人工的歪み（CH-2推進力）を完全除去し、**CH-4（ラバン・エフォート4要素: Weight, Time, Space, Flow）と10チャンネルボリューム**に基づいた純粋で自然な 3D 4ノード物理メッシュ運動を再構築しました：

$$\vec{P}_i(t) = (X_i(t), Y_i(t), Z_i(t)) \quad (i = 0, 1, 2, 3)$$

- **CH-2 推進力の除去**: 人工的な過剰Y軸シーソーねじれを廃止し、オノマトペ本来の自然な広がり・沈み込み・旋回に専念。
- **純粋な CH-4 エフォート力学**:
  - **Weight (重さ, x1)**: 全体が重力で下へ優雅に沈み込む（Gravity Sag）。
  - **Space (空間, x3)**: 4ノードの水平・3D旋回膨らみ半径（$R_{\text{orbit}}$）。
  - **Flow (流動, x4)**: 6本のばね結合エッジ (Pairwise Elastic Edges) のマイルドな粘性抵抗と自然な伸縮（赤: 引張 `d: 4.2` / 紫: 圧縮 `d: 2.1`）。
- **視認性最適化**: ノード径を大きく可視化し、ドラッグ操作による 3D 自由視点回転を維持。
