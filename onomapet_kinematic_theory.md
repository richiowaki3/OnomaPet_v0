...
## 2. 4ノード CH-2 逆位相3Dシーソー物理メッシュモデル (4-Node Opposing-Phase Seesaw Mesh)

ユーザーのダイナミックな試作提案に基づき、**4つの動点ノード (Node 0, 1, 2, 3)** に対し、CH-2 (推進モータ力) を **逆位相（Opposing Dual-Phase Seesaw Dynamics）** でアサインしました：

$$\vec{P}_i(t) = (X_i(t), Y_i(t), Z_i(t)) \quad (i = 0, 1, 2, 3)$$

- **N0, N2 (対角ノード対 A)**: CH-2 の推進力で **$+$ Y 方向（上方 UP）** へ押し上げ。
- **N1, N3 (対角ノード対 B)**: CH-2 の推進力で **$-$ Y 方向（下方 DOWN）** へ引き下げ。
- **6本の動的ばね結合エッジ (Pairwise Elastic Coupling Edges)**: N0/N2 が上がり N1/N3 が下がると、対角・全外周のエッジが動的に引き伸ばされ（赤色 `d: 4.8` テンション）↔ 圧縮され（紫色 `d: 1.8`）、**構造全体がダイナミックにグニャリとうねる3D立体シーソー運動**を表現。
- **視認性向上**: ノード径を大きく強調 (`radius = 12~20px`) し、ノードごとに `N0 (+Y)`, `N1 (-Y)` の位相タグを表示。
- **インタラクティブ 3D オービット**: ドラッグ操作で 3D 空間内の全アングルから立体視回転視察が可能。

---

## 3. 5チャンネル地震計スクロールチャート構造 (5-Channel Seismograph Renderer)
