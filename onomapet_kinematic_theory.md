...
## 2. 単一ノード 3D ラバン・エフォート表現モデル (Single 3D Dynamic Core Node for CH-4)

4ノードの分散を廃し、**単一のコア動点 (1 Single Dynamic Core Node)** に絞り込むことで、CH-4 (Laban Effort 4要素) の力学運動を高解像度で100%立体表現します：

$$\vec{P}(t) = (X(t), Y(t), Z(t))$$

- **Space (空間, x3)**: 緑色の **3D 空間旋回ガイドリング ($R_{\text{orbit}}$)** を形成。Direct (直線) $\leftrightarrow$ Indirect (迂回) に応じて1ノードの3D軌道半径が変化。
- **Time (時間, x2)**: **3D 運動軌跡ストリーム (Motion Trail)** の速度・加速度・インパルス波形を決定。
- **Weight (重さ, x1)**: 赤色の **3D 重力沈み込み線 (Gravity Sag Line)** と地面衝突時の **扁平・変形（Squish & Stretch）** を可視化。
- **Flow (流動, x4)**: 1ノードの背後に伸びる **シアン色の粘性残像トレイル (Viscous Aura Trail)** の長さと減衰。
- **インタラクティブ 3D オービット**: 画面上の 3D キャンバスをドラッグ操作することで、3D 空間内の任意の視点アングルから1ノードの立体運動を回転視察可能。
