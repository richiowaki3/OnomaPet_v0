...
## 2. 3D 4ノード離着物理メッシュ構造 (3D 4-Node Inter-Particle Elastic Mesh)

本システムは、**4つの動点ノード (Node 0, 1, 2, 3)** を 3D 遠近投影空間内に配置し、**6本のペアワイズ可変張力・ばねエッジ (6 Pairwise Coupling Edges)** で連結しています：

$$\vec{P}_i(t) = (X_i(t), Y_i(t), Z_i(t)) \quad (i = 0, 1, 2, 3)$$

- **離脱・拡大運動 ($D_{ij} \uparrow$)**: 10チャンネルボリュームの「旋回・広がり $\text{VOL}_2$」が高まると、4ノードが相互に遠ざかり、つなぐエッジが伸びてテンション表示（赤色）。
- **接近・収縮運動 ($D_{ij} \downarrow$)**: 「脈動・圧力 $\text{VOL}_5$」が高まると、4ノードがきゅっと中央へ急接近し、エッジが縮む（紫色）。
- **インタラクティブ 3D オービット**: 画面上の 3D キャンバスをドラッグ操作することで、3D 空間内の任意の視点アングルから4ノードの離着運動を立体視観察可能。
